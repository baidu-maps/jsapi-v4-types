declare namespace BMap {
  /**
   * FillLayer 支持的事件名与事件对象类型映射
     */
  type FillLayerEventMap = NormalLayerEventMap<FillLayer>;
  ...
  class FillLayer {
     ...
     */
    addEventListener<K extends keyof FillLayerEventMap>(event: K, handler: (e: FillLayerEventMap[K]) => void): void;
    /**
     * 移除事件监听
     * @param event 事件名称
     * @param handler 事件处理函数
     */
    removeEventListener<K extends keyof FillLayerEventMap>(event: K, handler: (e: FillLayerEventMap[K]) => void): void;
  }
}
