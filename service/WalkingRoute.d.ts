declare namespace BMap {
  ...
  class WalkingRoute {
     ...
     * const walkingRoute = new BMap.WalkingRoute(map, {
     *   renderOptions: { map: map, autoViewport: true },
     *   onSearchComplete(results) {
     *     console.log('路线已自动绘制到地图');
     *   },
     * });
     * walkingRoute.search('天安门', '王府井');
     * ```
     * @example
     * ```typescript
     ...
     * ```typescript
     * walkingRoute.search('天安门', '王府井');
     * walkingRoute.search(new BMap.Point(116.391, 39.910), new BMap.Point(116.431, 39.931));
     * ```
     */
     ...
     * @example
     * ```typescript
     * walkingRoute.setSearchCompleteCallback((results) => {
     ...
     * @example
     * ```typescript
     * walkingRoute.setMarkersSetCallback((pois) => {
     ...
     * @example
     * ```typescript
     * walkingRoute.setInfoHtmlSetCallback((poi, html) => {
     ...
     * @example
     * ```typescript
     * walkingRoute.setPolylinesSetCallback((polylines) => {
     *   polylines.forEach(pl => pl.setStrokeColor('green'));
     * });
     * ```
     */
     ...
     * @example
     * ```typescript
     * walkingRoute.setResultsHtmlSetCallback((container) => {
